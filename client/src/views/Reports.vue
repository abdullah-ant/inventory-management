...
export default {
  ...
  setup() {
    ...
    // Summary stats as computed properties — reactive to data reloads
    ...
    const loadData = async () => {
      // Reset error so a stale message doesn't persist across filter reloads
      error.value = null
      loading.value = true
      try {
    ...
    /**
     * Locale-aware month label from a YYYY-MM string.
     * Guards against invalid input; mirrors Dashboard's formatDate pattern.
     */
    ...
    // Max height is 200px; guard against division by zero when data is empty
    ...
    // Format month-over-month revenue change with correct currency symbol/conversion
    const getChangeValue = (current, previous) => {
      const change = current - previous
      return (change >= 0 ? '+' : '-') + formatCurrency(Math.abs(change), currentCurrency.value)
    }
    ...
    // Reload whenever the global filter bar changes (mirrors Dashboard)
    ...
    return {
      ...
      totalOrders,
      bestQuarter,
      // Expose currentCurrency as selectedCurrency to match Dashboard convention
...
/* Layout overrides only — visual (bg, shadow, border) handled by global .card */
.card {
  padding: 1.5rem;
...
.bar {
  width: 100%;
  background: linear-gradient(to top, var(--accent), var(--link));
...
.bar:hover {
  background: linear-gradient(to top, var(--highlight), var(--accent));
  opacity: 0.9;
}
...
/* Accent left-border + layout padding; bg/shadow handled by global .stat-card */
.stat-card {
  padding: 1.5rem;
...
/* Badge styles removed — global .badge.success/warning/danger handles dark tint style */
...
/* .loading and .error removed — global handles them with dark Dracula style */
</style>
